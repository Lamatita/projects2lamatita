...
async function handleAPI(request, env, path) {
  ...
  if (path === '/api/auth/register' && method === 'POST') {
    ...
    if (!username || !password) return jsonResponse({ message: 'Données invalides' }, 400);

    const existing = await db.prepare('SELECT id FROM users WHERE username = ?').bind(username).first();
    if (existing) return jsonResponse({ message: "Ce nom d'utilisateur est déjà pris" }, 409);
  ...
  if (path === '/api/auth/login' && method === 'POST') {
    const body = await request.json();
    const { username, password } = body;
    if (!username || !password) return jsonResponse({ message: 'Données requises' }, 400);
  ...
  if (path === '/api/auth/logout' && method === 'POST') {
    ...
    return jsonResponse({ message: 'Déconnecté' }, 200, { 'Set-Cookie': clearSessionCookie() });
  }

  if (path === '/api/auth/me' && method === 'GET') {
    const user = await getSessionUser(request, db);
    if (!user) return jsonResponse({ message: 'Non connecté' }, 401);
  ...
  if (path === '/api/groups/join' && method === 'POST') {
    ...
    if (!group) return jsonResponse({ message: 'Groupe non trouvé' }, 404);

    const already = await db.prepare('SELECT id FROM group_members WHERE group_id = ? AND user_id = ?').bind(group.id, user.id).first();
    if (already) return jsonResponse({ message: 'Vous êtes déjà membre' }, 409);
  ...
  if (path === '/api/scores' && method === 'POST') {
    ...
    if (typeof timeSeconds !== 'number' || !timerMode) return jsonResponse({ message: 'Données invalides' }, 400);
